...
with open('futajson/furnpickup _@310039170792030211_.json', 'r') as f:
    src_pick=json.load(f)
...
class Example(commands.Cog):
    ...
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.guild_only()
    async def pickupline(self,ctx, member: discord.Member = None):
        a=random.choice(src_pick)
        if not member:
          b =f"** {ctx.author.name} **says {a}"
        else:
          b =f"**{ctx.author.name}** says to **{member.name}**, {a}"
        await ctx.send(b)
